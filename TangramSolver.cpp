...
bool TangramSolver::solve(const cv::Mat &unitsImg, const cv::Mat &dstImg, std::vector<std::vector<cv::Point> > &resultPos)
{
    //double runtime = cv::getTickCount();
    //runtime = (cv::getTickCount() - runtime) / cv::getTickFrequency();
    //std::cout<<"runtime:"<<runtime<<std::endl;
    
    if(unitsImg.empty() || dstImg.empty())
        return false;
    
    //预处理之阈值化
    cv::Mat binUnitsImg, binDstsImg;
    myThreshold(unitsImg,binUnitsImg);
    myThreshold(dstImg,binDstsImg);
